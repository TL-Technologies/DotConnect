...
public class IAPManager : SingletonComponent<IAPManager>, ISaveable, IStoreListener
	{
		#region Classes
	...
	//// Failed
	//		[System.Serializable]
	//	public class OnProductFailedEvent : UnityEngine.Events.UnityEvent {}

	//	[System.Serializable]
	//	public class FailedEvent
	//	{
	//		public string					productId				= "";
	//		public OnProductFailedEvent	onProductPurchasedEvent	= null;
	//	}
		...
		Debug.Log("Start");
			...
			Debug.Log("Adding product to builder, id: " + productInfo.productId + ", consumable: " + productInfo.consumable);

				builder.AddProduct(productInfo.productId, productInfo.consumable ? ProductType.Consumable : ProductType.NonConsumable);
			}

		Debug.Log("Initializing IAP now...");
		...
		{
		Debug.Log("Initializion failed! Reason: " + failureReason);
		}

		public void OnPurchaseFailed(Product product, PurchaseFailureReason failureReason)
		{
		Debug.Log("Purchase failed for product id: " + product.definition.id + ", reason: " + failureReason);
		}
		...
		Debug.Log("BuyProduct: Purchasing product with id: " + productId);
			...
			if (IsInitialized)
			{
				...
				if (product == null)
				{
				Debug.Log("BuyProduct: product with id \"" + productId + "\" does not exist.");
				}
				else if (!product.availableToPurchase)
				{
				Debug.Log("BuyProduct: product with id \"" + productId + "\" is not available to purchase.");
				}
				else
				{
			...
			{
			Debug.Log("BuyProduct: IAPManager not initialized.");
			}
		}
		...
		Debug.Log("RestorePurchases: Restoring purchases");
			...
			if (IsInitialized)
			{
				...
				{
					Debug.Log("RestorePurchases: Device is not iOS, no need to call this method.");
				}
			}
		}
