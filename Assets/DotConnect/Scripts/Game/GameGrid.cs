...
namespace GameBug.DotConnect
{
    public class GameGrid : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
    {
        ...
        public void OnPointerDown(PointerEventData eventData)
        {
            ...
            Tutorials.instance.tutHand.gameObject.SetActive(false);
        ...
        {
            var PreviousSelectedBundle = PlayerPrefs.GetInt("PreviousSelectedBundle")-1;
            for (int i = 0; i <= PreviousSelectedBundle; i++)
            {
                for (int j = 0; j <= PreviousSelectedBundle; j++)
                {
                    if (grid[i][j].gridCell.endUnConnected.enabled)
                    {
                        var num = Random.Range(0, 13);
                        grid[i][j].gridCell.SetHint(true);
                    }
                }
            }
        ...
        public void HideColorBlind()
        {
            ...
            for (int i = 0; i <= PreviousSelectedBundle; i++)
            {
                for (int j = 0; j <= PreviousSelectedBundle; j++)
                {
                       grid[i][j].gridCell.SetHint(false);
                }
            }
            //Debug.LogError("Clicked");
        }
        ...
        private void ShowLineForHint(PlacedLine lineToShow)
        {
            ...
            grid[lineToShow.EndCell1.cellPos.row][lineToShow.EndCell1.cellPos.col].gridCell.num = Random.Range(0, 13);
            var num = grid[lineToShow.EndCell1.cellPos.row][lineToShow.EndCell1.cellPos.col].gridCell.num;
            grid[lineToShow.EndCell2.cellPos.row][lineToShow.EndCell2.cellPos.col].gridCell.num = num;
        ...
        private void DrawLineEndPoint(PlacedLine placedLine, LineCell endCell, LineCell lineSegmentCell, bool drawPlacedImages)
        {
            ...
            //var num = Random.Range(0,13);
            //cell.gridCell.hint.sprite = cell.gridCell.blind[num];
            //Debug.LogError(cell.gridCell.hint.sprite);
        }
        ...
        {
            GameManager.Instance.DeductHints();
        }
