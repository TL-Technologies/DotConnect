...
namespace GameBugEntertainment
{
    /**
    ...
    public class DailyRewardsInterface : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                LeanTween.moveLocalX(PanelDailyRewards, 2000f, 0.5f).setEaseInElastic().setDelay(0.25f).setOnComplete(() =>
                {
                    canvas.gameObject.SetActive(false);
                    LeanTween.moveLocalX(PanelDailyRewards, 0f, 0.5f);
                });
                MainMenuAnimations.instance.OnAnimatedUIIn();
            });
        ...
        private void CheckTimeDifference ()
        {
            if (!readyToClaim)
            {
                ...
                textTimeDue.text = string.Format("Come back in <b><color=#{0}>{1}</color></b> for your next reward!", ColorUtility.ToHtmlStringRGB(nextRewardColor), formattedTs);
            }
        }
